...
@router.post("/start", response_model=ExecutionResponse, status_code=status.HTTP_202_ACCEPTED)
async def start_execution(
    ...
    execution = await service.create_execution(payload.workflow_id, payload.input_payload, user)
...
async def list_executions(
    ...
    return await ExecutionService(db).list_for_user(user)
...
async def get_execution(
    ...
    execution = await ExecutionService(db).get_owned(execution_id, user)
    if execution is None:
...
async def get_execution_logs(
    ...
    logs = await ExecutionService(db).get_logs(execution_id, user)
    if logs is None:
        raise HTTPException(status_code=404, detail="Execution not found")
