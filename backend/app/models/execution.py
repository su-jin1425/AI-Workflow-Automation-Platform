...
class WorkflowExecution(Base, UUIDMixin, TimestampMixin):
    ...
    execution_status: Mapped[str] = mapped_column(String(30), default="queued", nullable=False)
    input_payload: Mapped[dict] = mapped_column(JSON, default=dict, nullable=False)
    output_payload: Mapped[dict | None] = mapped_column(JSON, nullable=True)
    started_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    completed_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    execution_logs: Mapped[list] = mapped_column(JSON, default=list, nullable=False)

    workflow = relationship("Workflow", back_populates="executions")
    node_executions = relationship("NodeExecution", back_populates="execution", cascade="all,delete")
    metrics = relationship("ExecutionMetric", back_populates="execution", cascade="all,delete")


class NodeExecution(Base, UUIDMixin, TimestampMixin):
    __tablename__ = "node_executions"

    execution_id: Mapped[UUID] = mapped_column(PG_UUID(as_uuid=True), ForeignKey("workflow_executions.id"), nullable=False)
...
class ExecutionMetric(Base, UUIDMixin, TimestampMixin):
    ...
    execution_id: Mapped[UUID] = mapped_column(PG_UUID(as_uuid=True), ForeignKey("workflow_executions.id"), nullable=False)
    execution_time: Mapped[float] = mapped_column(Float, default=0, nullable=False)
